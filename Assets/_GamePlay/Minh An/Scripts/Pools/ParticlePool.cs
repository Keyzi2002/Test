...
public static class ParticlePool
{
    ...
    public static void Play(ParticleSystem particle, Vector3 position, Quaternion rotation, Transform parentClone = null)
    {
        if (!particlePools.ContainsKey(particle) || particlePools[particle] == null)
        {
            particlePools.Add(particle, new Particle(particle, DEFAULT_POOL, parentClone));
        }

        particlePools[particle].Play(position, rotation);
    }
    public static void Release(ParticleSystem particle)
    {
        if (particlePools.ContainsKey(particle))
        {
            particlePools[particle].Release();

        }
        particlePools.Remove(particle);
    }
    ...
    public class Particle
    {
        ...
        public void Play(Vector3 position, Quaternion rotation)
        {
            ...
            if (particleSystems[index] == null || particleSystems[index].isPlaying)
            {
