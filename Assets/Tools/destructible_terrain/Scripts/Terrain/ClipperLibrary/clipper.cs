...
namespace ClipperLib
{
    ...
    internal struct Int128
    {
        ...
        private static bool negate;
        private static UInt64 a;
        private static UInt64 b;
        private static UInt64 c;
        private static UInt64 _lo;
        private static Int64 _hi;
        private static Int128 result;
        private static UInt64 int1Hi;
        private static UInt64 int1Lo;
        private static UInt64 int2Hi;
        private static UInt64 int2Lo;
        ...
        public static Int128 Int128Mul(Int64 lhs, Int64 rhs)
        {
            ...
            unchecked { _lo = (c << 32) + b; }
            if (_lo < b) _hi++;
            result = new Int128(_hi, _lo);
            return negate ? -result : result;            
        }

        private static Int128 cntr;
        public static Int128 operator /(Int128 lhs, Int128 rhs)
        {
            if (rhs.lo == 0 && rhs.hi == 0)
                throw new ClipperException("Int128: divide by zero");
             negate = (rhs.hi < 0) != (lhs.hi < 0);
            if (lhs.hi < 0) lhs = -lhs;
            if (rhs.hi < 0) rhs = -rhs;
    ...
    public class ClipperBase
    {
        ...
        //------------------------------------------------------------------------------
        private bool result;
        public bool AddPolygons(Polygons ppg, PolyType polyType, Action actionRoll, Action ResetActionComplete)
        {
            result = false;
            
            for (int i = 0; i < ppg.Count; ++i)
                if (AddPolygon(ppg[i], polyType, actionRoll, ResetActionComplete)) result = true;
        ...
        private int len;
        private Polygon p;
        private Int64 maxVal;
        public bool AddPolygon(Polygon pg, PolyType polyType, Action actionRoll, Action ResetActionComplete)
        {
            len = pg.Count;
    ...
    public class Clipper : ClipperBase
    {
        ...
        //------------------------------------------------------------------------------
        private JoinRec jr = new JoinRec();
        ...
        private void InsertLocalMinimaIntoAEL(Int64 botY)
        {
          while(  m_CurrentLM != null  && ( m_CurrentLM.Y == botY ) )
          {
            ...
            if (rb.outIdx >= 0 && rb.dx == horizontal)
            {
                ...
                {
                    pt = new IntPoint();
                    pt2 = new IntPoint(); //used as dummy params.
                    hj = m_HorizJoins[i];
        ...
        //------------------------------------------------------------------------------
        private OutPt op;
        private PolyNode pn = new PolyNode();
        ...
        //------------------------------------------------------------------------------
        private OutRec outRec1;
        private OutRec outRec2;
        private OutPt pp1a;
        private OutPt pp2a;
        private IntPoint pt1;
        private IntPoint pt3;
        private IntPoint pt4;
        private OutPt p3, p4, prev;
        ...
        private class PolyOffsetBuilder
        {
            ...
            private Polygon arc;
            private Clipper clpr = new Clipper();
            private IntRect r;
                ...
                JoinType jointype, double limit = 0, bool AutoFix = true)
            {
                ...
                clpr.Clear();
                clpr.AddPolygons(solution, PolyType.ptSubject, null, null);
                if (delta > 0)
                {
                ...
                else
                {
                    ...
                    clpr.AddPolygon(outer, PolyType.ptSubject, null, null);
            ...
            //------------------------------------------------------------------------------
            private IntPoint pt1;
            private IntPoint pt2;
            private double a1;
            private double a2;
            private double dx;
            internal void DoSquare(double mul)
            {
                pt1.x = (Int64)Round(pts[m_i][m_j].x + normals[m_k].X * delta);
                pt1.y = (Int64)Round(pts[m_i][m_j].y + normals[m_k].Y * delta);

                pt2.x = (Int64)Round(pts[m_i][m_j].x + normals[m_j].X * delta);
                pt2.y = (Int64)Round(pts[m_i][m_j].y + normals[m_j].Y * delta);

                if ((normals[m_k].X * normals[m_j].Y - normals[m_j].X * normals[m_k].Y) * delta >= 0)
                {
                    a1 = Math.Atan2(normals[m_k].Y, normals[m_k].X);
                    a2 = Math.Atan2(-normals[m_j].Y, -normals[m_j].X);
                    a1 = Math.Abs(a2 - a1);
                    if (a1 > Math.PI) a1 = Math.PI * 2 - a1;
                    dx = Math.Tan((Math.PI - a1) / 4) * Math.Abs(delta * mul);
                    pt1.x = (Int64)(pt1.x - normals[m_k].Y * dx);
                    pt1.y = (Int64)(pt1.y + normals[m_k].X * dx);

                    AddPoint(pt1);
                    pt2.x = (Int64)(pt2.x + normals[m_j].Y * dx);
                    pt2.y = (Int64)(pt2.y - normals[m_j].X * dx);
                    AddPoint(pt2);
                }
                else
                {
            ...
            internal void DoMiter()
            {
                ...
                {
                    double q = delta / m_R;
                    x.x = (Int64)Round(pts[m_i][m_j].x + (normals[m_k].X + normals[m_j].X) * q);
                    x.y = (Int64)Round(pts[m_i][m_j].y + (normals[m_k].Y + normals[m_j].Y) * q);
                    AddPoint(x);
                }
                else
                {
            ...
            {
                pt1.x = Round(pts[m_i][m_j].x + normals[m_k].X * delta);
                pt1.y = Round(pts[m_i][m_j].y + normals[m_k].Y * delta);

                pt2.x = Round(pts[m_i][m_j].x + normals[m_j].X * delta);
                pt2.y = Round(pts[m_i][m_j].y + normals[m_j].Y * delta);
                ...
                if ((normals[m_k].X * normals[m_j].Y - normals[m_j].X * normals[m_k].Y) * delta >= 0)
                {
                    if ((normals[m_j].X * normals[m_k].X + normals[m_j].Y * normals[m_k].Y) < 0.985)
                    {
                        ...
                        arc = BuildArc(pts[m_i][m_j], a1, a2, delta, Limit);
        ...
        //------------------------------------------------------------------------------
        private static Polygons result2 = new Polygons();
        private static Clipper c = new Clipper();
        public static Polygons SimplifyPolygon(Polygon poly, 
              PolyFillType fillType = PolyFillType.pftEvenOdd)
        {
            result2.Clear();
            c.Clear();
            c.AddPolygon(poly, PolyType.ptSubject, null, null);
            c.Execute(ClipType.ctUnion, result2, fillType, fillType);
            return result2;
        }
        //------------------------------------------------------------------------------
            ...
            PolyFillType fillType = PolyFillType.pftEvenOdd)
        {
            result2.Clear();
            c.Clear();
            c.AddPolygons(polys, PolyType.ptSubject, null, null);
            c.Execute(ClipType.ctUnion, result2, fillType, fillType);
            return result2;
        }
        //------------------------------------------------------------------------------
        ...
        private static Polygon result_;
        private static IntPoint pt_;
        private static int highI;
        private static bool UseFullRange;
            ...
            double distance = 1.415)
        {
            ...
            d = (int)(distance * distance);
            highI = poly.Count -1;
            while (highI > 0 && PointsAreClose(poly[highI], poly[0], d)) highI--;
            result_ = new Polygon(highI + 1);
            if (highI < 2) return result_;
            ...
            for (;;)
            {
                ...
                if (PointsAreClose(pt_, poly[j], d))
                {
                    i = j + 1;
                    while (i <= highI && PointsAreClose(pt_, poly[i], d)) i++;
                    continue;
                }

                if (PointsAreClose(poly[i], poly[j], d) ||
                    ClipperBase.SlopesEqual(pt_, poly[i], poly[j], UseFullRange)) 
                {
                    i = j;
                    continue;
                }
            ...
            if (i <= highI) result_.Add(poly[i]);
            i = result_.Count -1;
            if (i > 1 && ClipperBase.SlopesEqual(result_[i -1], result_[i], result_[0], UseFullRange))
                result_.RemoveAt(i);
            if (result_.Count < 3) result_.Clear();
            return result_;
        }
        //------------------------------------------------------------------------------
